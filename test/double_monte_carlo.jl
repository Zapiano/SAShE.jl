@testset "Double Monte Carlo: correctness vs. independent-linear closed form" begin
    ...
    S = DoubleMonteCarloSample(factor_names, dists, 3000, MonteCarloSampling(); N_V=8000)
...
@testset "Double Monte Carlo: regression vs. exact estimator (independent Ishigami)" begin
    ...
    S = DoubleMonteCarloSample(factor_names, dists, 3000, MonteCarloSampling(); N_V=5000)
    Φₙ, Φ²ₙ, _ = analyze(m, S)
    Φ = SAShE.shapley_effects(Φₙ)
...
@testset "Double Monte Carlo: error path for N_I < 2" begin
    factor_names = [:x1, :x2, :x3]
    dists = fill(Normal(), 3)
    @test_throws ArgumentError DoubleMonteCarloSample(
...
@testset "Double Monte Carlo: wrap constructor rejects a mismatched sample table" begin
    ...
    S = DoubleMonteCarloSample(factor_names, dists, 5, strategy; N_V=100, N_O=1, N_I=2)
    ...
    @test DoubleMonteCarloSample(S.samples, S.permutations, S.N_V, S.N_O, S.N_I) isa
        DoubleMonteCarloSample
end
