...
Container for the "callable model, plus an existing dataset" case — see the
[Examples](@ref) page for the use case this targets. Estimation logic not yet implemented —
this type exists so callers can construct it, but no `analyze` method is defined for it yet.
